...
The best way to get started with Mininet is to install our ready-to-run virtual machine image from the [Mininet-Getting-Started](Mininet-Getting-Started) page, and then go through the [Mininet-Walkthrough](Mininet-Walkthrough.md) and the [OpenFlow tutorial](http://www.openflow.org/wk/index.php/OpenFlow_Tutorial).
...
It's probably easiest to start out with a [VM install](Mininet-Getting-Started) of Mininet - it's pretty much guaranteed to work!

Instructions for native installation can be found [here](Mininet-Getting-Started) as well.
