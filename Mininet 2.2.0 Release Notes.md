...
* The Stanford Reference Switch (`--switch user`) has absolutely horrible performance (kilobits per second rather than megabits) on certain recent Linux kernels, including the one that we ship in the Mininet VM. We hope this can be fixed in the future.
