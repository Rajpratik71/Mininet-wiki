...
*Note that `host.pexec()` (like `host.cmd()`) runs commands as root, so this isn't really something you want to expose to the whole internet! But it's quite convenient,  isn't it? In a real example you would probably want a method to shut down both the REST server and the Mininet network in a graceful manner rather than using control-C and `mn -c`.]*
