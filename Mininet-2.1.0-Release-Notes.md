...
* Currently one of the bandwidth limit tests in `test_hifi.py` fails (or sometimes fails) due to low bandwidth in OVS in user mode on Ubuntu 13.10 (development build); if you run into this problem, the workaround is to use kernel-mode OVS.
